...
@interface RTHPersonViewController ()<UIScrollViewDelegate>
...
@implementation RTHPersonViewController
...
- (void)viewDidLoad {
    ...
    NSArray *controllers = @[[RTHdynamicsTableViewController class],[RTHCardPageTableViewController class],[RTHReplyPageTableViewController class]];
...
- (void)changTopMargin:(NSNotification *)noti {
    ...
    if (offset.y > -316 && offset.y < 0) {
        ...
        self.topViewTopMargin.constant = -1;
        self.navBarView.alpha = 0;
    }
}
...
- (SYSlideSelectedView *)slideView {
    if (_slideView == nil) {
        _slideView = [[SYSlideSelectedView alloc] initWithTitles:@[@"动态(2)",@"帖子(2)",@"回帖(2)"] frame:CGRectMake(0,90, ScreenW, 45) normalColor:[UIColor sy_colorWithRGB:0x000000] andSelectedColor:[UIColor sy_colorWithRGB:0xde2418]];
